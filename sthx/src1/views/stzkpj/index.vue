     ...
     <!-- <Box title="GDP/人口" >
      ...
      <Box title="EI指数">
        <div class="weater-box" slot="content" style="width: 100%; position: absolute; top: 30%;">
          <WeaterItem :style="{'width':weathersEIPJ.length===3?'30%':'40%'}"
                      :name="item.name" :value="item.value" :unite="item.unite" v-for="item in weathersEIPJ" :key="item.id" />
        </div>
      </Box>
...
export default {
      ...
      return{
        ...
        colorArr: {
         // "ei":['#ff0000','#ff6400','#C0FF3E','#66CD00','#015501'],
           "ei":['#b4533c','#c1a173','#9da385','#558848','#264321'],
  ...
  methods: {
    ...
    created() {
      this.queryDataCache1("2016","610000");
...
console.log('here')
      ...
      if(selFeature){
        ...
        //  fill: new ol.style.Fill({color: "rgba(255, 91, 121, 0.7)"}),
          fill: new ol.style.Fill({color: "rgba(14,97,160,0.5)"}),
...
/*// text 画图
    ...
    echartsDataExchange(DSJSXZDM){
        ...
        for(var key in stzkpjTable.stzkpjTableArr){
          ...
          if(fieldname == 'ei'){
            this.weathersEIPJ=new Array();
            for(var obj in this.hjjczzData[name])
            {
              this.weathersEIPJ.push(this.hjjczzData[name][obj]);
            }
            //this.weathersEIPJ = this.hjjczzData[name];
            var obj=new Object();
            obj.name="EI分级";
            if(this.weathersEIPJ[0].value>75)
            {obj.value="优";}
            else if(this.weathersEIPJ[0].value>55)
            {obj.value="良";}
            else if(this.weathersEIPJ[0].value>35)
            { obj.value="一般";}
            else if(this.weathersEIPJ[0].value>20)
            { obj.value="较差";}
            else
              { obj.value="差";}
            obj.unite="";
            this.weathersEIPJ.push(obj);
          }
          if(fieldname == 'wrfhzs'){
              ...
              if (resultData.status == "success") {
                ...
                for (var q = 0; q < stzkpjDataArr.length; q++) {
                  ...
                  // var yAxisValue = parseFloat(stzkpjDataArr[q].sthjzkzsei);
                    var yAxisValue = stzkpjDataArr[q].pjjg;
                  }
      ...
      //秦岭、流域
        ...
        for(var i=0;i<this.barSthjzleiData.xAxis.length;i++){
          ...
          //  pjNum = this.barSthjzleiData.yAxis[i];
            pjNum = this.barSthjzleiData.zAxis[i];
          }
        }
      }
      return {

...
<template>
  <div class="mainview">
    ...
    <div class="leftdiv">
        <Box title="林业" style="flex:1">
<!--          <barSthjzs slot="content" class="sheetstyle" :data="barSthjzsData"/>-->
        </Box>
        <Box title="污染源" style="flex:1">
<!--          <barSthjzs slot="content" class="sheetstyle" :data="barSthjzsData"/>-->
        </Box>
        <Box title="水电站" style="flex:1">
...
<!--    </div>-->
    ...
    <div class="rightdiv">
      <Box title="生态环境现状" style="flex:1">
<!--        <lineEIZS  slot="content" class="sheetstyle" :data="lineEIZSData"/>-->
      </Box>
      <Box title="生态环境指数变化" style="flex:1">
<!--        <lineEIZS  slot="content" class="sheetstyle" :data="lineEIZSData"/>-->
      </Box>

      <Box title="生物丰富度变化" style="flex:1">
<!--        <lineEIZS  slot="content" class="sheetstyle" :data="lineEIZSData"/>-->
      </Box>
    </div>
  </div>
...
/*      // 添加ArcGIS WMTS服务
    ...
    async loadSxxj(){
      ...
      async loadyjcld() {
          ...
          let json = require("../../assets/js/geojson/Json/保护站/sjbhz.json");
          //设置geojson线条、填充样式。load方法是个promise，用异步处理。
      ...
      async loadPOI(leixing){
          ...
          if(!this.geoCoordMapNew){
              ...
              for (var i = 0; i < entities.length; i++) {
                  ...
                  let xzqhmc = entity.properties.上级机构._value;
          ...
          for(var key in this.geoCoordMapNew){
              ...
              for(var i = 0; i < entities.length; i++){
                  ...
                  var xzqhmc = entities[i].properties.上级机构._value;
                  if(xzqhmc == key){
                      num++;
                  }
              }

...
export default {
  ...
  methods: {
    initChart() {
      var that = this;
      var tipValue = this.data.tooltip;
      ...
      var lableFormatter = "";
      if(tipValue == "Rk"){
        tooltipFormatter = "{b} : {c} 万人 ({d}%)"
        lableFormatter = "{b} : " +"\n {c} 万人({d}%)"
      }
      else if(tipValue == "GDP"){
        tooltipFormatter = "{b} : {c} 亿元 ({d}%)"
        lableFormatter = "{b} : " +"\n {c} 亿元({d}%)"
      }
      let option = {
        title: [
        ...
        color: ["#32ec7c","#43dce8","#f6cf2f","#7a9df1", "#43dce8","#564AA3", "#4cc2ff", ],
        //color: [,,"#66FF66"],
